...
class ShrineSearchForm(forms.Form):
    q = forms.CharField(required=False, label="キーワード")
    pref = forms.ChoiceField(required=False, label="都道府県", choices=PREF_CHOICES)
    sort = forms.ChoiceField(
        required=False, label="並び替え",
        choices=[("name","名前順"),("built_year","古い順"),("nearest","現在地に近い順")],
        initial="name"
    )
    lat = forms.FloatField(required=False, widget=forms.HiddenInput)
    lng = forms.FloatField(required=False, widget=forms.HiddenInput)
    fav = forms.BooleanField(required=False, label="お気に入りのみ")
