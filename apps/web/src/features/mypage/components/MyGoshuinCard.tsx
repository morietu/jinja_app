...
export default function MyGoshuinCard({
  ...
  const handleClickCard = () => {
    console.log("[MyGoshuinCard] click", { id: item.id, shrine: item.shrine });
    onOpenDetail?.(item);
  };
  ...
  return (
    <Link href={`/shrines/${item.shrine}`} className="block">
      <article
        ...
        tabIndex={0}
        onClick={() => {
          console.log("ARTICLE onClick", item.id);
          onOpenDetail?.(item);
        }}
        ...
        <div className="relative aspect-[4/5] overflow-hidden bg-gradient-to-br from-muted to-muted/60">
          ...
          <div
            className={cn(
              ...
              "pointer-events-none", // ← 常時。カードクリックを邪魔しない
            )}
          >
            {onToggleVisibility && (
              <Button
