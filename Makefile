...
.PHONY: help \
	...
	seed-representative-docker \
	pr-create pr-status pr-open pr-edit-base \
	auth-users auth-reset-pass
...
help:
	...
	@echo "  PG_MODE=docker|local         # DBの実行先切替（既定 local）"
	@echo "make seed-representative-check # 代表枠の件数+サンプル確認"
	@echo "make seed-representative-list  # 代表枠一覧"
	@echo "make seed-representative-docker # docker DBで seed 実行"
	@echo ""
	@echo "make auth-users                # ユーザー一覧"
	@echo "make auth-reset-pass AUTH_USER=etsuko AUTH_PASS=testpass  # パスリセット"
	@echo ""
...
spectacular:
	GOOGLE_PLACES_API_KEY=$${GOOGLE_PLACES_API_KEY:-dummy} \
	GOOGLE_MAPS_API_KEY=$${GOOGLE_MAPS_API_KEY:-dummy} \
	$(PY) backend/manage.py spectacular --file api_schema.yaml
...
# ===== nogis targets (target-local export) =====
env-nogis: export USE_GIS := 0
env-nogis: export DISABLE_GIS_FOR_TESTS := 1
env-nogis:
	@unset DATABASE_URL; \
	$(PY) -m django check
...
test-nogis: export USE_GIS := 0
test-nogis: export DISABLE_GIS_FOR_TESTS := 1
...
seed-representative-docker:
	...
	@PG_MODE=docker $(MAKE) seed-representative
