...
def normalize_recs(js: Dict[str, Any], query: str = "") -> Dict[str, Any]:
    ...
    for it in items:
        ...
        reason = (
            it.get("reason") or it.get("特徴") or it.get("説明") or it.get("description") or ""
        ).strip()
        if name:
...
def _intent_reason(query: str) -> str:
    ...
    if has("恋愛", "縁結び", "えんむすび"):
        return "縁結び・恋愛成就で知られる神社です。"
    if has("学業", "合格", "勉学", "受験"):
        return "学業成就・合格祈願で参拝者が多い神社です。"
    if has("仕事", "商売", "出世", "昇進"):
        return "仕事運・商売繁盛のご利益で知られています。"
    if has("金運", "財運", "開運"):
        return "金運・開運のご利益で有名です。"
    if has("健康", "病気平癒", "安産"):
        return "健康祈願・安産祈願で信仰を集めています。"
    if has("厄除", "厄払い", "方位", "方角"):
        return "厄除け・方位除けの参拝に適しています。"
    return "ご利益で知られる神社です。"
...
def _pick_location_from_candidate(c: Dict[str, Any]) -> str:
    for key in (
        ...
        if isinstance(v, str) and v.strip():
            return v.strip()
    parts = [
        c.get(k, "") for k in ("prefecture", "city", "ward", "town", "chome", "block", "building")
    ]
