import json
import re
from .schemas import normalize_recs, complete_recommendations
...
def _extract_markdown_list(text: str):
    ...
    if not items:
        return None
    return {"recommendations": [{"name":it["name"], "location":"", "reason":it.get("reason","")} for it in items[:3]]}
class ConciergeOrchestrator:
    ...
    def suggest(self, query: str, candidates: List[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if isinstance(msg, dict) and msg.get("content"):
            ...
            tmp = (normalize_recs(data, query=query) if data is not None else (_extract_markdown_list(msg["content"]) or {"raw": msg["content"]}))
            return (complete_recommendations(tmp, query=query, candidates=candidates) if isinstance(tmp, dict) and 'recommendations' in tmp else tmp)
...
def chat_to_plan(message: str, lat: float, lng: float, transport: str = "walking"):
    """
    Back-compat function expected by existing views.
    Delegates to temples.llm.tools.orchestrator.chat_to_plan.
    """
    # 遅延 import で循環依存を回避
